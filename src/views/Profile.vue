            ...
            <div class="sm:col-span-3">
              ...
              <div class="mt-2">
                <input 
                    ...
                    name="username" 
                    type="text"
                    @blur="handleBlur('username', form.username)"
                    class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6" />
              </div>
            </div>
            <div class="sm:col-span-3">
              <label for="country" class="block text-sm/6 font-medium text-gray-900">Language</label>
            ...
            <div class="sm:col-span-3">
              ...
              <div class="mt-2">
                <input 
                    ...
                    name="first-name" 
                    id="first-name"
                    @blur="handleBlur('first_name', form.first_name)"
                    class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6" />
              </div>
            </div>
            ...
            <div class="sm:col-span-3">
              ...
              <div class="mt-2">
                <input 
                    ...
                    name="last-name" 
                    id="last-name" 
                    @blur="handleBlur('last_name', form.last_name)" 
                    class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6" />
              </div>
            </div>
...
const handleBlur = async (field, value) => {
  if (value === originalForm.value[field]) return;

  try {
    await authStore.updateProfile({ [field]: value});
    originalForm.value[field] = value;
    showToast.success(`${fieldNames[field]} updated successfully`);
  } catch (error) {
    showToast.error(`Failed to update ${fieldNames[field]}`);  
  }
};  
</script>